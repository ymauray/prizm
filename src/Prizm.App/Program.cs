...
using Debugger = Prizm.Core.Debugging.Debugger;
...
var snapshotFolder = Path.Combine(documents, "Prizm");
...
[
    ...
    new Menu("Help",
    [
        new MenuItem("About Prizm", KeyboardKey.Null, aboutBox.Open),
    ]),
]);

// A file can be given on the command line: dotnet run --project src/Prizm.App -- game.tap
if (args.Length > 0)
{
    Open(args[0]);
}
...
void SaveSnapshot()
{
    try
    {
        ...
        var path = Path.Combine(snapshotFolder, $"Prizm-{DateTime.Now:yyyyMMdd-HHmmss}.sna");
...
string NewRecordingPath(byte[] firstBlock)
{
    ...
    if (name.Length == 0)
    {
        name = $"Prizm-{DateTime.Now:yyyyMMdd-HHmmss}";
    }
