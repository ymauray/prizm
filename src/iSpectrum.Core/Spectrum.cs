...
/// <summary>
/// What every Spectrum model shares: a Z80, a ULA, a tape deck, and the frame loop that runs
/// them one 50 Hz frame, or one instruction, at a time. Models differ by their memory, their
/// ports and their timings.
/// </summary>
public abstract class Spectrum
{
    ...
    /// <summary>The model's clock, frame and contention timings.</summary>
    public SpectrumTimings Timings { get; }

    public Z80Cpu Cpu { get; }

    public virtual SpectrumMemory Memory => _memory;
    ...
    /// <summary>
    /// Whether the ROM holding the 48K BASIC tape routines (LD-BYTES) is paged in at 0x0000:
    /// always on the 48K; on the 128K, only when ROM 1 is.
    /// </summary>
    protected virtual bool IsTapeRomPaged => true;
